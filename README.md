...
docker run --rm \
  ...
  -w /github/workspace \
  ghcr.io/skaldlab/muninn:0.3.10 \
  --target . \
  --output json,sarif \
  --fail-on high
```
...
```bash
go install github.com/skaldlab/muninn@v0.3.10
```
...
```bash
docker buildx imagetools inspect ghcr.io/skaldlab/muninn:0.3.10 \
  --format '{{ json .SBOM }}'
docker buildx imagetools inspect ghcr.io/skaldlab/muninn:0.3.10 \
